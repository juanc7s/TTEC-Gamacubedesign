...
long unsigned int currentReadingTelemetryFileIndex = -1;
long unsigned int nextTelemetry;

void write_telemetry_to_json_doc(){
  for (int i=0;i<5;i++ ) {
    telemetry.msg1[i] = doc ["msg1"][i];
  }
  for (int i=0;i<5;i++ ) {
    telemetry.msg2[i] = doc ["msg2"][i];
  }
  ...
  *(float*)telemetry.curr = doc ["Curr"];
  currentWritingTelemetryFileIndex = doc ["index"];
...
void telemetry_send(){
  ...
  read_telemetry_from_SD(readingFile);
}
